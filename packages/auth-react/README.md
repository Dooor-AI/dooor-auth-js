...
`dooorAuthMiddleware` decrypts and authenticates the first-party AES-GCM session cookie with Web Crypto, including Edge runtimes, and rejects forged, corrupted, or structurally invalid cookies. Central revocation, refresh, and business authorization remain in the BFF and server route handlers. Post-login redirects are restricted to the mini app's own origin, and `signOut()` revokes the central IdP refresh session before clearing the local cookie.
